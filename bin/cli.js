...
const args = require('meow')(`
    ...
    Options
      ...
      -C, --coin <coin>              crypto-coin to enable [default: BZX]
      ...
      -P, --bitcoind-port <port>     port for bitcoind rpc [default: 29201]
    ...
    All options may also be specified as environment variables
      $ BZXEXP_PORT=8080 BZXEXP_BITCOIND_PORT=18443 BZXEXP_BITCOIND_COOKIE=~/.bitcoin/regtest/.cookie btc-rpc-explorer
...
Object.keys(args).filter(k => k.length > 1).forEach(k => {
  if (args[k] === false) process.env[`BZXEXP_NO_${envify(k)}`] = true;
  else process.env[`BZXEXP_${envify(k)}`] = args[k];
})
