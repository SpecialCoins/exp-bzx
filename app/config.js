...
var currentCoin = process.env.BZXEXP_COIN || "BZX";
...
var cookieSecret = process.env.BZXEXP_COOKIE_SECRET
...
var electrumXServerUriStrings = (process.env.BZXEXP_ELECTRUMX_SERVERS || "").split(',').filter(Boolean);
...
["BZXEXP_DEMO", "BZXEXP_PRIVACY_MODE", "BZXEXP_NO_INMEMORY_RPC_CACHE"].forEach(function(item) {
	if (process.env[item] === undefined) {
		process.env[item] = "false";
	}
});

["BZXEXP_NO_RATES", "BZXEXP_UI_SHOW_TOOLS_SUBHEADER"].forEach(function(item) {
	if (process.env[item] === undefined) {
...
module.exports = {
	...
	privacyMode: (process.env.BZXEXP_PRIVACY_MODE.toLowerCase() == "true"),
	demoSite: (process.env.BZXEXP_DEMO.toLowerCase() == "true"),
	queryExchangeRates: (process.env.BZXEXP_NO_RATES.toLowerCase() != "true"),
	noInmemoryRpcCache: (process.env.BZXEXP_NO_INMEMORY_RPC_CACHE.toLowerCase() == "true"),
	
	rpcConcurrency: (process.env.BZXEXP_RPC_CONCURRENCY || 10),

	rpcBlacklist:
	  process.env.BZXEXP_RPC_ALLOWALL  ? []
	: process.env.BZXEXP_RPC_BLACKLIST ? process.env.BZXEXP_RPC_BLACKLIST.split(',').filter(Boolean)
	: [
		"addnode",
		"backupwallet",
		"bumpfee",
	...
	addressApi:process.env.BZXEXP_ADDRESS_API,
	electrumXServers:electrumXServers,

	redisUrl:process.env.BZXEXP_REDIS_URL,
	...
	site: {
		...
		header:{
			showToolsSubheader:(process.env.BZXEXP_UI_SHOW_TOOLS_SUBHEADER == "true"),
			dropdowns:[
				{
					title:"Related Sites",
					links:[
	...
	donations:{
		addresses:{
			coins:["BZX"],
			sites:{"BZX":"https://explorer.btc21.org"},

			"BZX":{address:"3NPGpNyLLmVKCEcuipBs7G4KpQJoJXjDGe"}
		},
		btcpayserver:{
