...
if (process.env.BZXEXP_BASIC_AUTH_PASSWORD) {
	app.disable('x-powered-by');
	app.use(auth(process.env.BZXEXP_BASIC_AUTH_PASSWORD));
}
...
app.continueStartup = function() {
	...
	if (config.addressApi) {
		...
		if (!supportedAddressApis.includes(config.addressApi)) {
			utils.logError("32907ghsd0ge", `Unrecognized value for BZXEXP_ADDRESS_API: '${config.addressApi}'. Valid options are: ${supportedAddressApis}`);
		}
		...
		if (config.addressApi == "electrumx") {
			if (config.electrumXServers && config.electrumXServers.length > 0) {
				...
				utils.logError("327hs0gde", "You must set the 'BZXEXP_ELECTRUMX_SERVERS' environment variable when BZXEXP_ADDRESS_API=electrumx.");
			}
		}
	}
