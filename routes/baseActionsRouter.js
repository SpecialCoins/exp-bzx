...
router.get("/rpc-terminal", function(req, res, next) {
	if (!config.demoSite && !req.authenticated) {
		res.send("RPC Terminal / Browser require authentication. Set an authentication password via the 'BZXEXP_BASIC_AUTH_PASSWORD' environment variable (see .env-sample file for more info).");
